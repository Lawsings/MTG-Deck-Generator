...
export async function generate(opts) {
  ...
  // Pas de filtre dur pour la mécanique : on booste au score.
  function synergyBonus(c) {
    ...
    return Math.min(0.4, hits * 0.1); // 0.1 par mécanique matchée, max 0.4
  }
  ...
  // -> buildManabase doit retourner EXACTEMENT targetLands lands (≤ 8 non-basiques)

  // 7) Caler EXACTEMENT la taille : 1 cmdr + (99 = nonlands + lands)
  // Les LANDS sont déjà fixés par buildManabase => on complète *uniquement* en sorts.
  ...
  if (nonlandsFinal.length < desiredNonlands) {
    ...
    for (const c of pool) {
      ...
      const price = priceEUR(c);
      if (deckBudget > 0 && price > deckBudget * 1.2) continue; // garde-fou budget doux pour le top-up
      remainder.push(c);
    }
